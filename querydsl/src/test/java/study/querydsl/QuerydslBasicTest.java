...
import static com.querydsl.jpa.JPAExpressions.*; // 서브 쿼리 작성을 위한 static
import static org.assertj.core.api.Assertions.*;
import static study.querydsl.entity.QMember.member; // QMember 한 번에 가져와서 사용하기!!! => 추천
...
public class QuerydslBasicTest {
    ...
    @BeforeEach // Test실행전 먼저 실행됨
    public void before(){
        queryFactory = new JPAQueryFactory(em); // em를 생성자로 넣어준다.
    ...
    public void startQuerydsl(){
        // member1을 찾아라. => Querydsl => 시작이 new JPAQueryFactory(em); 임
//        QMember m = new QMember("m"); => m은 "select m from Member m where m.username = :username" 의 m과 같은 것임.
//        QMember m = QMember.member; 가능
    ...
    public void searchAndParam(){
        ...
        Member findMember = queryFactory
                .selectFrom(member)
                .where(
                        member.username.eq("member1"), // and 대신에 ,로 chaining가능 (추천!!)
                        member.age.eq(10))
                .fetchOne();
        assertThat(findMember.getUsername()).isEqualTo("member1");
    }
    ...
    public void group() throws Exception {
        List<Tuple> result = queryFactory
                ...
                .from(member)
                .join(member.team, team) // member의 team과 team을 join한다.
                .groupBy(team.name)
                .fetch();

        Tuple teamA = result.get(0); // tuple를 조회하는 방법
    ...
    public void theta_join() throws Exception {
        ...
        List<Member> result = queryFactory
                .select(member)
                .from(member, team) // 연관관계가 없는 두 필드를 조인할 수 있긴함...
    ...
    @PersistenceUnit
    EntityManagerFactory emf; // getPersistenceUnitUtil()을 사용하기 위해
    ...
    public void fetchJoinUse(){
        ...
        Member findMember = queryFactory
                .selectFrom(member)
                .join(member.team, team).fetchJoin() // fetchJoin() => LAZY인 값을 한 번에 조회해온다.
                .where(member.username.eq("member1"))
                .fetchOne();
    ...
    public void subQuery() throws Exception {
        ...
        QMember memberSub = new QMember("memberSub"); // 서브쿼리를 작성하고 구별하기 위해 기존의 member와 다르게 memberSub로 엘리어스 설정

        List<Member> result = queryFactory
                .selectFrom(member)
                .where(member.age.eq( // 쿼리 안에 또 하나의 쿼리 작성법 JPAExpressions 활용!
    ...
    @Test // Case문 => 잘안씀.. 이런거는 app의 logic에서 해결하는 것이 나음
    ...
    public void concat(){
        List<String> result = queryFactory
                .select(member.username.concat("_").concat(member.age.stringValue())) // stringValue() 자주 쓰임.. ENUM타입 등을 String로 바꿀때
                .from(member)
                .where(member.username.eq("member1"))
                .fetch();
